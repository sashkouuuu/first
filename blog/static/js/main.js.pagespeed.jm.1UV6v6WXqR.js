...
$(document).ready(function () {
    ...
    function setBannerPosition() {
        if ($('.wide-gallery').length) {
            var wideGalleryBanner = $('.wide-gallery__banner'), wideGalleryImage = $('.wide-gallery__images'),
            ...
            wideGalleryBanner.css({'top': top + 'px', 'left': left + 'px', 'right': 'auto'});
        }
    }
    ...
    function setWideGallery() {
        if ($('.wideGallery').length) {
            var windowWidth = $(window).width(), wysiwygOffsetLeft = $('.wysiwyg').offset().left,
    ...
    $('[autofocus]').focus();
    var body = $('body'), search_btn = $('.search'), search_block = $('.searchBlock'),
    ...
    body.on('click', '.menu', function () {
        var menu = $('.side_menu_wrap'), close_btn = $('.side_menu_wrap .close_btn'), data = $(this).data();
        if (data.source) {
            if (menu.length) {
                showSideMenu();
                showOverlay();
    ...
    var countIteration = 0, adriver_banner_1 = $('#adriver_banner_1'), addriverSetInterval;
    if (adriver_banner_1.length > 0) {
        addriverSetInterval = setInterval(function () {
            if (countIteration > 10 || adriver_banner_1.height() > 0) {
                countIteration++;
                setBannerPosition();
                clearInterval(addriverSetInterval)
            }
        }, 300);
    }
});